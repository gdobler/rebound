...
def get_mn_lidar_tiles():
    """
    Get the LiDAR tiles with centers within the Manhattan boundaries.
    """

    # -- get the lidar tile list and convert to geometries
    cfile  = os.path.join(os.environ["REBOUND_WRITE"], "lidar_tile_info.csv")
    coords = pd.read_csv(cfile)

    # -- convert coords to polygons
    xmin, xmax, ymin, ymax = coords[["xmin", "xmax", "ymin", "ymax"]].values.T
    polys = [Polygon(((i, k), (i, l), (j, l), (j, k), (i, k))) for 
             i, j, k, l in zip(xmin, xmax, ymin, ymax)]

    # -- get the borough boundaries
    bfile = os.path.join(os.environ["REBOUND_DATA"], "nyc_shp", "nybb.shp")
    boros = gp.read_file(bfile)
    boros = boros[boros.BoroName == "Manhattan"]

    # -- return tiles that intersect Manhattan
    tiles = gp.GeoDataFrame({"geometry" : polys})

    return coords[tiles.geometry.intersects(boros.iloc[0].geometry)]
...
def rasterize_mn_lidar():
    """
    Rasterize the MN LiDAR tiles.
    """

    # -- get the lidar tile names and range
    coords = get_mn_lidar_tiles()
    flist  = coords.filename.values
    nfiles = len(flist)
    mm     = [[coords.xmin.min(), coords.ymin.min()], 
              [coords.xmax.max(), coords.ymax.max()]]
