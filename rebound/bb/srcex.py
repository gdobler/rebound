...
def convert_lum(data_dir):
    '''
    Takes as input full filepath for directory of Oct 31, 2013 raw data images
    
    Reads raw image file, reshapes into hard coded dimensions 
    for images from October 2013: 2160 by 4096 pixels in RGB format
    ...
    '''
    return  np.array([np.fromfile(os.path.join(data_dir,i),dtype=np.uint8).reshape(2160,4096,3).mean(axis=-1) for i in sorted(os.listdir(data_dir))[::60]])
